...
class Config:
    ...
    USER_AGENT = os.getenv("USER_AGENT", "VulnLabScanner/1.5.0")
    MAX_RETRIES = int(os.getenv("MAX_RETRIES", "3"))

    # Configuración de Autenticación
    DEFAULT_LOGIN_PATHS = os.getenv("DEFAULT_LOGIN_PATHS", "/login,/signin,/auth,/login.php").split(",")

    # Configuración de Access Control
    DEFAULT_ADMIN_PATHS = os.getenv("DEFAULT_ADMIN_PATHS", "/admin,/administrator,/admin/users,/dashboard/admin").split(",")

    # Configuración de CSRF
    CSRF_TOKEN_PATTERNS = os.getenv(
        "CSRF_TOKEN_PATTERNS", "csrf_token,csrfmiddlewaretoken,_csrf,token,authenticity_token"
    ).split(",")
